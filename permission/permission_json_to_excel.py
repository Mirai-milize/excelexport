...
def convert_json_to_excel():
    ...
    for payload in data:
        if isinstance(payload, dict) and 'users' in payload:
            row_data = payload.get('users')
            if isinstance(row_data, list):
                records_to_process.extend(row_data)
            else:
                print("경고: 'users' 키의 값이 리스트가 아닙니다. 해당 항목을 건너뜁니다.")
        else:
    ...
    if not records_to_process:
        print("오류: JSON 데이터에서 'users' 키를 찾을 수 없거나 처리할 레코드가 없습니다.")
        return

    keys_to_extract = ['name', 'ikenId', 'mobile']
